...
//parameterは後から数値変更
    ...
    this->declare_parameter("weight_dist1_", 1.1);
    this->declare_parameter("weight_dist2_", 0.7);
    this->declare_parameter("weight_heading1_", 0.5);
    this->declare_parameter("weight_heading2_", 0.3);
    this->declare_parameter("radius_margin1",0.5);
    this->declare_parameter("radius_margin2",1.0);
    this->declare_parameter("roomba_radius",0.35);

    this->declare_parameter("max_accel_",1000.0);
    this->declare_parameter("max_dyawrate_",1000.0);

    this->declare_parameter("turn_theres_yawrate_", 0.25);
    this->declare_parameter("avoid_theres_vel_", 0.25);

    this->declare_parameter("dt_", 0.1);
    this->declare_parameter("goal_tolerance_",0.5);
    this->declare_parameter("predict_time_",2.0);
    this->declare_parameter("vel_reso_",0.05);
    this->declare_parameter("yawrate_reso_",0.05);
    ...
    tf_listener_ = std::make_shared<tf2_ros::TransformListener>(*tf_buffer_,this,50);


    // ####### Subscriber #######
    sub_local_goal_ = this->create_subscription<geometry_msgs::msg::PointStamped>("/local_goal", rclcpp::QoS(1).reliable(),std::bind(&DWAPlanner::local_goal_callback,this,std::placeholders::_1));
    sub_obs_poses_ = this->create_subscription<geometry_msgs::msg::PoseArray>("/o_points", rclcpp::QoS(1).reliable(),std::bind(&DWAPlanner::obs_poses_callback,this,std::placeholders::_1));
    // sub_odom_ = this->create_subscription<nav_msgs::msg::Odometry>(
    // "/odom",  
    // rclcpp::QoS(10),
//     std::bind(&DWAPlanner::odom_callback, this, std::placeholders::_1)
// );
...
void DWAPlanner::local_goal_callback(const geometry_msgs::msg::PointStamped::SharedPtr msg)
{
    ...
    try//try-catchで失敗しそうなコードを安全に処理できる
    {
        ...
        transform = tf_buffer_->lookupTransform(
            robot_frame_,  // "base_link" などロボット座標系 mapじゃね？
...
// void DWAPlanner::odom_callback(const nav_msgs::msg::Odometry::SharedPtr msg)
// {
//     roomba_.velocity = msg->twist.twist.linear.x;
//     roomba_.yawrate  = msg->twist.twist.angular.z;
//     // RCLCPP_INFO(this->get_logger(), "Got odom! vel=%.2f, yaw=%.2f", roomba_.velocity, roomba_.yawrate);
// }
...
bool DWAPlanner::can_move()
{
    ...
    // RCLCPP_INFO(this->get_logger(),"dx = %.2f,%.2f\n",local_goal_.point.x,roomba_.x);
    // RCLCPP_INFO(this->get_logger(),"dy = %.2f,%.2f\n",local_goal_.point.y,roomba_.y);
    // RCLCPP_WARN(this->get_logger(),"distance = %.2f,%.2f,%.2f\n",distance_to_goal,dx,dy);
...
{
    cmd_speed_.mode=11;
    // RCLCPP_INFO(this->get_logger()," x , yaw = %.2f , %.2f",velocity,yawrate);
...
std::vector<double> DWAPlanner::calc_final_input()
{
    ...
    // RCLCPP_INFO(this->get_logger(),"min,max,%.2f,%.2f,%.2f,%.2f",dw_.min_vel,dw_.max_vel,dw_.min_yawrate,dw_.max_yawrate);
    ...
    for (double v = dw_.min_vel; v <= dw_.max_vel; v += vel_reso_)
    {
        for (double w = dw_.min_yawrate; w <= dw_.max_yawrate; w += yawrate_reso_)
        {
            ...
            visualize_traj(trajectories[i], pub_predict_path_, this->now());
            if(v<0.1 && abs(w)<0.1)
            {
                total_score = -1e6;
            }
    ...
    if (max_score <= -1e6) {
        RCLCPP_WARN(this->get_logger(), "All trajectories scored too low. Using fallback input.");
        input = {0.01, 0.5}; // 少しだけ旋回させるとか
        }
    // 現在速度の記録
    ...
    // ###### pathの可視化 #######
    RCLCPP_INFO(this->get_logger(),"v w = %.2f %.2f",input[0],input[1]);
    visualize_traj(trajectories[index_of_max_score], pub_optimal_path_, this->now());
    // RCLCPP_INFO_STREAM(get_logger(), "input");
...
void DWAPlanner::calc_dynamic_window()
{
    ...
    // RCLCPP_INFO(this->get_logger(),"vel = %.2f,yaw = %.2f",vel,yawrate);
    // ###### 車両モデルによるWindow ######
    // 設計上の最大速度・最小速度の制約
    ...
    double Vd[] = {
        vel - max_accel_ * dt_, // 最小速度　std::maxで二つを比較できる
        vel + max_accel_ * dt_, // 最大速度
        yawrate - max_dyawrate_ * dt_, // 最小旋回速度
        yawrate + max_dyawrate_ * dt_  // 最大旋回速度
    };
    // RCLCPP_INFO(this->get_logger(),"calc");
...
double DWAPlanner::calc_dist_eval(const std::vector<State>& traj)
{
    ...
    for (const auto& state : traj)
    {
        ...
        {
            double dx =  obs_pose.position.x - state.x;
            double dy =  obs_pose.position.y - state.y;
            double dist = sqrt(dx * dx + dy * dy); // ユークリッド距離
      //      RCLCPP_INFO(this->get_logger(),"dist = %.2f",dist);

            if (dist < roomba_radius_ + safety_margin) {      // 衝突とみなす
                min_dist = -1e6;//衝突したら飛ばして論外
		return min_dist;
            }
            if (dist < min_dist)
            {
    ...
    //        RCLCPP_INFO(this->get_logger(),"dist = %.2f",min_dist);
        }
    }
...
void DWAPlanner::visualize_traj(const std::vector<State>& traj, rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr pub_local_path, rclcpp::Time now)
{
   ...
   // RCLCPP_INFO(this->get_logger(),"visual");
