...
{ 
    ...
    this->declare_parameter("init_yaw", 3.0); // 新マップで更に逆向きに変更
    ...
    this->declare_parameter("init_yaw_dev", 0.6);
...
void Localizer::broadcast_odom_state()
{
    ...
    if(flag_broadcast_)
    {
        // RCLCPP_INFO(this->get_logger(), "last_odom: position=(%.2f, %.2f)",
        ...
        double mx = estimated_pose_.x();
        double my = estimated_pose_.y();
        double myaw = estimated_pose_.yaw();
        // map→baseの位置ベクトルを取得（絶対位置）
        double ox = last_odom_.pose.pose.position.x;
        double oy = last_odom_.pose.pose.position.y;
        double oyaw = tf2::getYaw(last_odom_.pose.pose.orientation);
        // base→odomの相対位置をmap座標系に変換（回転を考慮）
        double theta = normalize_angle(myaw - oyaw); 
        double dx = mx - (ox * cos(theta) - oy * sin(theta));
        double dy = my - (ox * sin(theta) + oy * cos(theta));
        // yaw差分（回転の向き）を取得
        // double yaw_odom =  // yawを取り出す
        // double dyaw = theta - yaw_odom;
        // dyaw = // 回転に関しては計算後に適切な範囲に変更

        // (サンプルコードに記述済み)
        tf2::Quaternion map_to_odom_quat;
        map_to_odom_quat.setRPY(0, 0, theta);
        ...
        // RCLCPP_INFO(this->get_logger(),"odom pub");
